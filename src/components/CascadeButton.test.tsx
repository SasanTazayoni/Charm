...
describe("CascadeButton", () => {
  ...
  it("applies the cascade-button class", () => {
    const { container } = render(<CascadeButton variant="gold">Click me</CascadeButton>);
  ...
  it("applies additional className", () => {
    const { container } = render(<CascadeButton variant="gold" className="my-class">Click me</CascadeButton>);
  ...
  describe("cascade effect", () => {
    ...
    it("appends squares container after mount", () => {
      const { container } = render(<CascadeButton variant="gold">Click</CascadeButton>);
    ...
    it("applies pink color to squares for pink-outline variant", () => {
      const { container } = render(<CascadeButton variant="pink-outline">Click</CascadeButton>);
      const square = container.querySelector(".cascade-square") as HTMLElement;
    ...
    it("reveals square at cursor position on mouseenter", () => {
      ...
      // clientX=10, clientY=10 → col=2, row=2 → index=42
    ...
    it("does not reveal squares on mouseenter when disabled", () => {
      const { container } = render(<CascadeButton variant="gold" disabled>Click</CascadeButton>);
      const button = container.querySelector("button")!;
      fireEvent.mouseEnter(button, { clientX: 10, clientY: 10 });
      const squares = container.querySelectorAll(".cascade-square");
      const anyRevealed = Array.from(squares).some((s) => (s as HTMLElement).style.opacity === "0");
    ...
    it("reveals square at cursor position on mousemove while hovering", () => {
      ...
      const button = container.querySelector("button")!;
      fireEvent.mouseEnter(button, { clientX: 10, clientY: 10 });
      // clientX=50, clientY=20 → col=10, row=4 → index=90
    ...
    it("expands wave to adjacent squares after interval tick", () => {
      ...
      fireEvent.mouseEnter(button, { clientX: 10, clientY: 10 });
      const squares = container.querySelectorAll(".cascade-square");
      const before = Array.from(squares).filter((s) => (s as HTMLElement).style.opacity === "0").length;
      act(() => { vi.advanceTimersByTime(20); });
      const after = Array.from(squares).filter((s) => (s as HTMLElement).style.opacity === "0").length;
    ...
    it("clears interval when all squares are revealed", () => {
      ...
      const button = container.querySelector("button")!;
      fireEvent.mouseEnter(button, { clientX: 10, clientY: 10 });
      act(() => { vi.advanceTimersByTime(20 * 200); });
      const squares = container.querySelectorAll(".cascade-square");
      const revealed = Array.from(squares).filter((s) => (s as HTMLElement).style.opacity === "0").length;
    ...
    it("clears interval on mouseleave and stops wave expansion", () => {
      ...
      // All squares should be back to opacity 1 (or empty), none stuck at 0
      const squares = container.querySelectorAll(".cascade-square");
      const stillRevealed = Array.from(squares).filter((s) => (s as HTMLElement).style.opacity === "0").length;
    ...
    it("does not re-reveal a square already in the wave", () => {
      ...
      const button = container.querySelector("button")!;
      fireEvent.mouseEnter(button, { clientX: 10, clientY: 10 });
      // Move to the same cell — startWave should bail out early
    ...
    it("removes squares container on unmount", () => {
      const { container, unmount } = render(<CascadeButton variant="gold">Click</CascadeButton>);
      expect(container.querySelector(".squares-container")).toBeTruthy();
      unmount();
